...
export default function DashboardPage() {
  ...
  const upcomingBills = recurringBills.filter(b => b.status === 'due' || b.status === 'overdue').slice(0,3);
    ...
    <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
      <Card className="lg:col-span-1">
        <CardHeader className="pb-2">
          ...
          <CardTitle className="text-4xl text-primary">{formatCurrency(balance, currency)}</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="text-xs text-muted-foreground">
            {formatCurrency(totalIncome, currency)} income vs {formatCurrency(totalExpenses, currency)} expenses
          </div>
        </CardContent>
        <CardFooter>
      ...
      <Card className="lg:col-span-3">
        <CardHeader>
          ...
          <CardDescription>Top 5 spending categories this period.</CardDescription>
        </CardHeader>
        <CardContent className="h-[300px] w-full">
          <ChartContainer config={chartConfig} className="h-full w-full">
            <ResponsiveContainer width="100%" height="100%">
                <BarChart data={spendingChartData} layout="vertical" margin={{ right: 20 }}>
                  <CartesianGrid horizontal={false} />
                  <XAxis type="number" dataKey="value" tickFormatter={(value) => formatCurrency(value, currency).replace(currency,"")} />
                  <YAxis type="category" dataKey="name" width={100} tickLine={false} axisLine={false} />
                  <ChartTooltip
                    cursor={false}
                    content={<ChartTooltipContent hideLabel />}
                  />
                  <Bar dataKey="value" fill="hsl(var(--primary))" radius={4} barSize={30} />
                </BarChart>
            </ResponsiveContainer>
          </ChartContainer>
        </CardContent>
        ...
        <Card key={pot.id}>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              {pot.icon ? <PiggyBankIcon className="h-6 w-6 text-primary" /> : <Target className="h-6 w-6 text-primary" />} 
              {pot.name}
            </CardTitle>
            <CardDescription>
          ...
          <CardContent>
            <Progress value={(pot.currentAmount / pot.goal) * 100} className="mb-2 h-3" />
          ...
          <CardFooter>
             <Link href="/savings" legacyBehavior passHref>
                <Button size="sm" variant="outline">Manage Pots</Button>
             </Link>
          </CardFooter>
        </Card>
      ))}
        ...
        <Card className="lg:col-span-1">
          ...
          <CardContent>
            {upcomingBills.map(bill => (
              <div key={bill.id} className="mb-3 pb-3 border-b last:border-b-0 last:mb-0 last:pb-0">
                ...
                <div className="text-sm text-muted-foreground">
                  {formatCurrency(bill.amount, currency)} due {formatDate(bill.nextDueDate)}
                </div>
              </div>
            ))}
          </CardContent>
          <CardFooter>
            <Link href="/bills" legacyBehavior passHref>
                <Button size="sm" variant="outline">View All Bills</Button>
             </Link>
          </CardFooter>
        </Card>
      )}
       ...
       <Card className="lg:col-span-2">
        ...
        <CardContent>
          <p className="text-sm text-muted-foreground mb-4">
            Analyze your spending habits and get AI-powered recommendations to improve your financial health.
          </p>
        </CardContent>
        <CardFooter>
