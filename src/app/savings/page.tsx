...
export default function SavingsPotsPage() {
  ...
  const handleTransaction = (type: 'add' | 'withdraw') => {
    ...
    toast({
      title: "Success",
      description: `Successfully ${type === 'add' ? 'added' : 'withdrew'} ${formatCurrency(amount, currency)} ${type === 'add' ? 'to' : 'from'} ${selectedPot.name}.`,
    });
        ...
        <Card key={pot.id} className="flex flex-col">
          <CardHeader>
            <div className="flex items-center justify-between mb-2">
              ...
              <Badge variant={pot.currentAmount >= pot.goal ? "default" : "secondary"} className={pot.currentAmount >= pot.goal ? "bg-green-500 text-white" : ""}>
          ...
          <CardFooter className="flex gap-2">
            <Dialog onOpenChange={(isOpen) => { if (!isOpen) setAmount(0); }}>
              ...
              <DialogContent>
                ...
                <DialogFooter>
                  ...
                  <DialogClose asChild>
                    <Button onClick={() => handleTransaction('add')} disabled={amount <=0}>Add Funds</Button>
                  </DialogClose>
                </DialogFooter>
              </DialogContent>
            </Dialog>
              ...
              <DialogContent>
                ...
                <DialogFooter>
                  <DialogClose asChild>
                   ...
                   <DialogClose asChild>
                    <Button onClick={() => handleTransaction('withdraw')} variant="destructive" disabled={amount <=0}>Withdraw Funds</Button>
                  </DialogClose>
                </DialogFooter>
              </DialogContent>
            </Dialog>
